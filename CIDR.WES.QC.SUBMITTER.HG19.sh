...
################################################################
	...
	CREATE_SAMPLE_ARRAY ()
	{
		...
		# Used for limited where to run base quality score recalibration on where to create gvcf files.
			...
			# there is no merged mendel or cutting bed file, for hg19
			# but it does not hurt to have it here, so keeping this as is.
...
################################################################################
	...
	################################################################################################
		...
		CALL_HAPLOTYPE_CALLER_GVCF_GATHER ()
		{
			...
			${HG19_SCRIPT_DIR}/G02-HAPLOTYPE_CALLER_GVCF_GATHER.sh \
				${GATK_3_7_0_CONTAINER} \
				${ALIGNMENT_CONTAINER} \
				${CORE_PATH} \
				${PROJECT} \
	...
	########################################################
		...
		CALL_HAPLOTYPE_CALLER_BAM_GATHER ()
		{
			...
			${HOLD_ID_PATH_GVCF_AND_HC_BAM_GATHER} \
			${HG19_SCRIPT_DIR}/G03-HAPLOTYPE_CALLER_BAM_GATHER.sh \
				${ALIGNMENT_CONTAINER} \
				${CORE_PATH} \
				${PROJECT} \
				${SM_TAG} \
...
# run end project functions (qc report, file clean-up) for each project
	...
	PROJECT_WRAP_UP ()
	{
		...
		${HG19_SCRIPT_DIR}/X01-X01-END_PROJECT_TASKS.sh \
			${CORE_PATH} \
			${ALIGNMENT_CONTAINER} \
			${PROJECT} \
			${SAMPLE_SHEET} \
