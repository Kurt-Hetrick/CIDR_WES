...
##################
	...
	# Generate a list of active queue and remove the ones that I don't want to use
		...
		QUEUE_LIST=`qstat -f -s r \
			...
			| sort \
			| uniq \
			| egrep -v "all.q|cgc.q|programmers.q|rhel7.q|bigmem.q|bina.q|qtest.q|bigdata.q" \
			| datamash collapse 1 \
			| awk '{print $1}'`
...
##############################################################################################################
##### DEPRECATING THIS; TOO MUCH LOAD ON QMASTER TO ADD THIS TO A SCATTER GATHER. ############################
##### SHOULD BE FAST ENOUGH SERIALLY OR CAN TEST GNU PARALLEL TO ADD SOME PARALLELIZATION WITHIN ONE JOB #####
##############################################################################################################
